...
jQuery( document ).ready(function($) {
    //Chosen JS
    $(".juju-blog-chosen-select").chosen({
        width: "100%"
    });

    // icon picker
    $('.juju-blog-icon-picker').each( function() {
    ...
    $( document ).on( 'click', '.customize_multi_add_field', juju_blog_customize_multi_add_field )
        .on( 'change', '.customize_multi_single_field', juju_blog_customize_multi_single_field )
        .on( 'click', '.customize_multi_remove_field', juju_blog_customize_multi_remove_field )
	...
	function juju_blog_customize_multi_add_field(e) {
		...
		if ( ! $this.data( 'lockedAt' ) || + new Date() - $this.data( 'lockedAt' ) > 300 ) {
			...
			juju_blog_customize_multi_write( $control );
		}
		$this.data( 'lockedAt', + new Date() );
	}

	function juju_blog_customize_multi_single_field() {
		var $control = $( this ).parents( '.customize_multi_input' );
		juju_blog_customize_multi_write( $control );
	}

	function juju_blog_customize_multi_remove_field(e) {
		e.preventDefault();
		var $this = $( this );
		var $control = $this.parents( '.customize_multi_input' );
		$this.parent().remove();
	...
	function juju_blog_customize_multi_write( $element) {
...
jQuery(document).ready(function($) {
    // Sortable sections
    jQuery( 'ul.juju-blog-sortable-list' ).sortable({
        handle: '.juju-blog-drag-handle',
        axis: 'y',
        update: function( e, ui ){
            jQuery('input.juju-blog-sortable-input').trigger( 'change' );
        }
    });

    /* On changing the value. */
    jQuery( "body" ).on( 'change', 'input.juju-blog-sortable-input', function() {
        /* Get the value, and convert to string. */
        this_checkboxes_values = jQuery( this ).parents( 'ul.juju-blog-sortable-list' ).find( 'input.juju-blog-sortable-input' ).map( function() {
            return this.value;
        }).get().join( ',' );

        /* Add the value to hidden input. */
        jQuery( this ).parents( 'ul.juju-blog-sortable-list' ).find( 'input.juju-blog-sortable-value' ).val( this_checkboxes_values ).trigger( 'change' );
...
( function( api ) {
    ...
    const  juju_blog_section_lists = ['featured_slider_section','trending_topics_section','about_section','most_recent_posts_section','most_read_posts_section'];
     juju_blog_section_lists.forEach( juju_blog_homepage_scroll );

    function juju_blog_homepage_scroll(item, index) {
         // Detect when the front page sections section is expanded (or closed) so we can adjust the preview accordingly.
        wp.customize.section( 'juju_blog_'+item, function( section ) {
    ...
    wp.customize( 'juju_blog_theme_options[reset_options]', function( setting ) {
        setting.bind( function( value ) {
            ...
            if ( value ) {
                setting.notifications.add( code, new wp.customize.Notification(
                    ...
                    {
                        type: 'info',
                        message: juju_blog_reset_data.reset_message
                    }
                ) );
            } else {
