...
if( class_exists( 'WP_Customize_Control' ) ) :
    class Trendy_Blog_WP_Repeater_Control extends WP_Customize_Control {
        public $id;
        ...
        public function __construct( $manager, $id, $args = array() ) {
            add_filter( 'repeater_input_labels_filter',array( $this, 'repeater_labels' ), 10 , 3 );

            parent::__construct( $manager, $id, $args );
            /*Get options from customizer.php*/
            $this->add_field_label = esc_html__( 'Add new field', 'trendy-blog' );
            if ( ! empty( $args['add_field_label'] ) ) {
                $this->add_field_label = $args['add_field_label'];
            }

            $this->boxtitle = esc_html__( 'Customizer Repeater', 'trendy-blog' );
            ...
            if ( file_exists( get_template_directory() . '/inc/customizer/custom-controls/repeater/icons.php' ) ) {
                $this->customizer_icon_container =  '/inc/customizer/custom-controls/repeater/icons';
            }
        ...
        */
        function repeater_labels( $string, $id, $control ) {
            if ( $id === 'social_icons' ) {
                if ( $control === 'customizer_repeater_checkbox_control' ) {
                    return esc_html__( 'Check to open link in new tab','trendy-blog' );
                }
            }
            return $string;
        }
        ...
        public function enqueue() {
            wp_enqueue_style( 'font-awesome', get_template_directory_uri().'/assets/lib/fontawesome/css/all.min.css', array(), '5.15.3' );

            wp_enqueue_style( 'trendy-blog-customizer-repeater', get_template_directory_uri().'/inc/customizer/custom-controls/repeater/repeater.css', array(), TRENDY_BLOG_VERSION );

            wp_enqueue_style( 'wp-color-picker' );

            wp_enqueue_script( 'trendy-blog--customizer-repeater', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/repeater.js', array('jquery', 'jquery-ui-draggable', 'wp-color-picker' ), TRENDY_BLOG_VERSION, true  );

            wp_enqueue_script( 'fontawesome-iconpicker', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/fontawesome-iconpicker.js', array( 'jquery' ), '4.6.3', true );

            wp_enqueue_style( 'fontawesome-iconpicker', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/fontawesome-iconpicker.min.css', array(), '4.6.3' );
        }
            ...
            if(!empty($array)){
                foreach($array as $icon){ ?>
                    <div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
                        <div class="customizer-repeater-customize-control-title" <?php if( $it == 0 ) echo 'repeater-expanded'; ?>>
                            <?php echo esc_html( $this->boxtitle ) ?>
                        </div>
                        <div class="customizer-repeater-box-content-hidden" <?php if( $it == 0 ) echo 'style="display: block"'; ?>>
                            <?php
                            $choice = $image_url = $icon_value = $title = $subtitle = $text = $text2  = $link2 = $link = $checkbox = $shortcode = $repeater = $color = $color2 = '';
                            if(!empty($icon->id)){
                                $id = $icon->id;
                            }
                            ...
                            if($this->customizer_repeater_color_control == true) {
                                $this->input_control(array(
                                    'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','trendy-blog' ), $this->id, 'customizer_repeater_color_control' ),
                            ...
                            if($this->customizer_repeater_color2_control == true){
                                $this->input_control(array(
                                    'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','trendy-blog' ), $this->id, 'customizer_repeater_color2_control' ),
                            ...
                            if($this->customizer_repeater_title_control==true){
                                $this->input_control(array(
                                    'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Title','trendy-blog' ), $this->id, 'customizer_repeater_title_control' ),
                            ...
                            if($this->customizer_repeater_subtitle_control==true){
                                $this->input_control(array(
                                    'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Subtitle','trendy-blog' ), $this->id, 'customizer_repeater_subtitle_control' ),
                            ...
                            if($this->customizer_repeater_text_control==true){
                                $this->input_control(array(
                                    'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Text','trendy-blog' ), $this->id, 'customizer_repeater_text_control' ),
                            ...
                            if($this->customizer_repeater_link_control){
                                $this->input_control(array(
                                    'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link','trendy-blog' ), $this->id, 'customizer_repeater_link_control' ),
                            ...
                            if($this->customizer_repeater_text2_control==true){
                                $this->input_control(array(
                                    'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Text','trendy-blog' ), $this->id, 'customizer_repeater_text2_control' ),
                            ...
                            if($this->customizer_repeater_link2_control){
                                $this->input_control(array(
                                    'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link','trendy-blog' ), $this->id, 'customizer_repeater_link2_control' ),
                            ...
                            if($this->customizer_repeater_checkbox_control==true){
                                $this->input_control(array(
                                    'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Checkbox','trendy-blog' ), $this->id, 'customizer_repeater_checkbox_control' ),
                            ...
                            if($this->customizer_repeater_shortcode_control==true){
                                $this->input_control(array(
                                    'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Shortcode','trendy-blog' ), $this->id, 'customizer_repeater_shortcode_control' ),
        ...
        private function input_control( $options, $value='' ) { ?>
            ...
            if( !empty($options['type']) ){
                switch ($options['type']) {
                    ...
                    case 'checkbox': ?>
                        <input type=checkbox class="<?php echo esc_attr( $options['class'] ); ?>" <?php checked( esc_attr($value) ); ?>/>
                        <span class="customize-control-title"><?php echo esc_html( $options['label'] ); ?></span>
                        <?php
                        break;
        ...
        private function icon_picker_control($value = '', $show = ''){
            ...
            <div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
                <span class="customize-control-title">
                    <?php esc_html_e('Icon','trendy-blog'); ?>
                </span>
        ...
        private function image_control($value = '', $show = ''){ ?>
            <div class="customizer-repeater-image-control" <?php if( $show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' || empty( $show ) ) { echo 'style="display:none;"'; } ?>>
                <span class="customize-control-title">
                    <?php esc_html_e('Image','trendy-blog')?>
                </span>
                <input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
                <input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image','trendy-blog' ); ?>" />
            </div>
            <?php
        }
        ...
        private function icon_type_choice($value='customizer_repeater_icon'){ ?>
            <span class="customize-control-title">
                <?php esc_html_e('Image type','trendy-blog');?>
            </span>
            <select class="customizer-repeater-image-choice">
                <option value="customizer_repeater_icon" <?php selected($value,'customizer_repeater_icon');?>><?php esc_html_e('Icon','trendy-blog'); ?></option>
                <option value="customizer_repeater_image" <?php selected($value,'customizer_repeater_image');?>><?php esc_html_e('Image','trendy-blog'); ?></option>
                <option value="customizer_repeater_none" <?php selected($value,'customizer_repeater_none');?>><?php esc_html_e('None','trendy-blog'); ?></option>
            </select>
            <?php
        }
        ...
        private function repeater_control($value = ''){
            ...
            $show_del        = 0; ?>
            <span class="customize-control-title"><?php esc_html_e( 'Social icons', 'trendy-blog' ); ?></span>
            <?php
            if(!empty($value)) {
                ...
                <div class="customizer-repeater-social-repeater">
                    <div class="customizer-repeater-social-repeater-container">
                        ...
                        <input type="text" class="customizer-repeater-social-repeater-link"
                            placeholder="<?php esc_attr_e( 'Link', 'trendy-blog' ); ?>">
                        <input type="hidden" class="customizer-repeater-social-repeater-id" value="">
                        <button class="social-repeater-remove-social-item" style="display:none">
                            <?php esc_html_e( 'Remove Icon', 'trendy-blog' ); ?>
                        </button>
                    </div>
                    <input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
                </div>
                <button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'trendy-blog' ); ?></button>
                <?php
            } else { ?>
                <div class="customizer-repeater-social-repeater">
                    <?php
                        ...
                        <div class="customizer-repeater-social-repeater-container">
                            ...
                            <button class="social-repeater-remove-social-item"
                                    style="<?php if ( $show_del == 1 ) {
                                        echo "display:none";
                                    } ?>"><?php esc_html_e( 'Remove Icon', 'trendy-blog' ); ?></button>
                        </div>
                        <?php
                    } ?>
