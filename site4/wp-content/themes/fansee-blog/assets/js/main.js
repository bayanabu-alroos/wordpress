...
document.addEventListener('keydown', function (e) {
    ...
    if (e.keyCode == 27) {
        ...
        const event = new Event('fanseeBlogOnEscKeypressed');
        document.dispatchEvent(event);
    }
});

document.addEventListener('fanseeBlogOnEscKeypressed', function () {
...
jQuery(document).ready(function () {
    ...
    if (jQuery().fstPopupSearch) {
        jQuery('.fansee-blog-toggle-search').fstPopupSearch({
    ...
    if (jQuery().slick) {
        jQuery('.fansee-blog-testimonial').slick();
        ...
        jQuery('.fansee-blog-feature-news-slider-init').slick({
